...
nps.invalid = function(Q,l,m,n, N = sum(Q), S = sum(Q), int_z){
  air = M_air(Q,l,m,n)
  excl = M_excl(Q,l,m,n, N = N, S = S, int_z = int_z)
...
M_excl = function(Q,l,m,n,N = sum(Q), S = sum(Q), int_z){
  ...
  Ry = matrix(unlist(rep(Ry,l)),n^m,l*m) # repeat columns to allow acces dependent on z
  ...
  # we have to increase the nested sampling steps and starting points
  # since we have more dimensions compared to the valid model estimation,
  # otherwise the estimated integral below will very likely always be smaller than
  # the valid one, independent of the data
  dim_dif = (m^l)*((n^m)+1) - (m^l)*((n^m)) #the relative difference between the dimensions of the valid and exclusion causal model
  new_N = N^dim_dif
  new_S =  S #ceiling(S*dim_dif)
  ...
  integers = apply(h, 1, function(hi){
    ...
    # Log-likelihood function for estimating the integral of theta_xy
    llf_xy = function(t){
      return(XY_product(unlist(t),P,Q))
    }
    Int_xy = estimate_integral(N=new_N,S=new_S,d=(m^l)*((n^m)+1),llf=llf_xy,sample_theta_xy)
    return(int_z * Int_xy)
  })
  return (max(integers))
}
...
I_naive = function(Q,b){
  ...
  denom_gsum = factorial((sum(Q+as.bigz(b))))
...
I_optimized = function(Q,b){
  ...
  d_sum_upper = sum(Q+as.bigz(4))
  d_sum_lower = q_max+b
  d_gb = gamma(b)^length(Q)

  result = product_fraction(unlist(lapply((Q_num+as.bigz(b)), gamma)),c(d_sum_lower:d_sum_upper, d_gb))
  return (result)
}
